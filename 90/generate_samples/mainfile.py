...
def main(configs):
    ...
    checkpoint = torch.load('model-128.pt')
    ...
    ddp_model.load_state_dict(bc_old)
    ...
    ddp_model.to(torch.device("cuda:0"))
   ...
   # gc.collect()
    ...
    if configs.max_epochs == 0:  # no training, just samples
        sample, time_ge = generation(configs, dataDims, ddp_model)
    ...
    #    print([configs.training_batch_size])
        while (epoch <= (configs.max_epochs + 1)) & (converged == 0):  # over a certain number of epochs or until converged
            err_tr, time_tr = model_epoch_new(configs, dataDims = dataDims, trainData = tr, model = ddp_model, optimizer = optimizer, update_gradients = True)  # train & compute loss
            err_te, time_te = model_epoch_new(configs, dataDims = dataDims, trainData = te, model = ddp_model, update_gradients = False)  # compute loss on test set
            ...
            if int(epoch % 2 == 0):
                with open('check' + str(epoch+100) + '.txt', 'w') as f:
         ...
         #   if configs.comet:
          ...
          #      sample0 = next(iter(tr))
           #     out = F.softmax(ddp_model(sample0.cuda().float()), dim=1).cpu().detach().numpy()
        ...
        save({
            ...
            'optimizer_state_dict': optimizer.state_dict()},'./model-amorphous-2.pt')
        sample, time_ge = generation(configs, dataDims, ddp_model)
       # log_generation_stats(configs, epoch, experiment, sample, agreements, output_analysis)
    print('finished!')