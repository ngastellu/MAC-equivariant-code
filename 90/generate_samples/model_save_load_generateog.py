...
from models import *
a_file = open("datadimsrelu.pkl","rb")
...
parser.add_argument('--fc_dropout_probability', type = float, default = 0.4) # dropout probability on hidden FC layer(s) [0,1)
...
parser.add_argument('--sample_outpaint_ratio', type = int, default = 3) # size of sample images, relative to the input images
...
checkpoint = torch.load('model-amorphous-65layer40filter-epoch50.pt', map_location=device)
...
if configs.sample_generation_mode == 'serial':
        ...
        for batch in range(batches):  # can't do these all at once so we do it in batches
            ...
            np.save('samples/run_{}_amorphoussamplestrimmed_model-amorphous-65layer4lowertemp'.format(configs.run_num), sample.cpu())
    
elif configs.sample_generation_mode == 'parallel':
    #    if configs.CUDA:
    #        cuda.synchronize()
     #   time_ge = time.time()
        dataDims['sample x dim']= dataDims['sample x dim']*3
        dataDims['sample y dim']=dataDims['sample y dim']*3
        ...
        for image in range(configs.n_samples):  # can't do these all at once so we do it in batches
            ...
            sample_batch.fill_(1)  # initialize with minimum value
                ...
                while finished_rows < (sample_y_padded):  # generate row-by-row
                    ...
                    for k in range(dataDims['channels']):  # actually do the generation
                       ...
                       # print(out[:, 1:, k, -1, dataDims['conv field']])
                        probs = F.softmax(out[:, 1:, k, -1, dataDims['conv field']] /0.6, dim=1).data  # the remove the lowest element (boundary)
            ...
            np.save('samples/newMACmodel'.format(configs.run_num), sample.cpu())
